...
def test_alice_battle_behavior():
    ...
    print(
        f"[OK] Step 1: 自身を対象に使用し、チャージが {state_1.get_env('alice_charge')} になりました。"
    )
    ...
    print(
        f"[OK] Step 2: 自身を対象に使用し、チャージが {state_2.get_env('alice_charge')} になりました。"
    )
    ...
    print(
        f"[OK] Step 3: チャージが上限 {state_3.get_env('alice_charge')} のまま維持されることを確認しました。"
    )
    ...
    print(
        "[OK] Step 4: 敵を対象に使用し、正常にサイクルしチャージがリセットされました。"
    )
