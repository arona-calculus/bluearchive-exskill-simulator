...
def test_rio_hanako_integration():
    ...
    initial_cards = ("Rio", "Hanako_Swimsuit", "Aru", "Kisaki", "Kikyou", "Sena")

    sim = Simulator(specs)
    state = sim.initialize_state(initial_cards)

    # --- Initial State ---
    assert state.cards == ("Rio", "Hanako_Swimsuit", "Aru", "Kisaki", "Kikyou", "Sena")
    ...
    # パッシブによりハナコのゲージが+40。
    state = sim.play(2)
    assert state.cards == ("Rio", "Hanako_Swimsuit", "Kisaki", "Kikyou", "Sena", "Aru")
    ...
    # リオはその場でRio_Copyに変身し、rio_copy_target="Hanako_Swimsuit"を記録する。
    state = sim.play(0, target="Hanako_Swimsuit")
    assert state.cards == ("Rio_Copy", "Hanako_Swimsuit", "Kisaki", "Kikyou", "Sena", "Aru")
    assert state.get_env("rio_copy_target") == "Hanako_Swimsuit"
    ...
    state = sim.play(0)
    assert state.cards == ("Kikyou", "Hanako_Swimsuit", "Kisaki", "Sena", "Aru", "Rio")
    ...
    # パッシブによりハナコのゲージが+40。
    state = sim.play(0)
    assert state.cards == ("Sena", "Hanako_Swimsuit", "Kisaki", "Aru", "Rio", "Kikyou")
    ...
    # パッシブによりハナコのゲージが+40。
    state = sim.play(0)
    assert state.cards == ("Aru", "Hanako_Swimsuit", "Kisaki", "Rio", "Kikyou", "Sena")
    ...
    # ゲージ60<100のため、通常サイクル。
    state = sim.play(1)
    assert state.cards == ("Aru", "Rio", "Kisaki", "Kikyou", "Sena", "Hanako_Swimsuit")
    assert state.get_env("hanako_gauge", 0) == 60
    print("[OK] Step 6: ハナコ自身がEXを使用。ゲージを100消費して手札に留まる（60）。")
