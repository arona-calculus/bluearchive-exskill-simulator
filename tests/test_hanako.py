...
def test_hanako_full_behavior():
    ...
    assert state_2.env["hanako_gauge"] == 2
    ...
    assert state_3.env["hanako_gauge"] == 4
    print(f"[OK] Step 3: Gauge increased to {state_3.env['hanako_gauge']} after Mutsuki's skill.")

    # --- Step 4: Reach max gauge ---
    state_4 = transition(state_3, 0)  # Use Kayoko
    assert state_4.cards == ("Hanako", "Haruka", "Hoshino", "Aru", "Mutsuki", "Kayoko")
    assert state_4.env["hanako_gauge"] == 6
    print(f"[OK] Step 4: Gauge reached max ({state_4.env['hanako_gauge']}) and Hanako returned to hand.")

    # --- Step 5: Use Hanako with SUFFICIENT gauge (6) ---
    state_5 = transition(state_4, 0)
    assert state_5.cards == ("Hanako", "Haruka", "Hoshino", "Aru", "Mutsuki", "Kayoko")
    assert state_5.env["hanako_gauge"] == 3
    print(f"[OK] Step 5: Hanako stayed in hand, gauge consumed to {state_5.env['hanako_gauge']}.")

    # --- Step 6: Use Hanako again with just enough gauge (3) ---
    state_6 = transition(state_5, 0)
    assert state_6.cards == ("Hanako", "Haruka", "Hoshino", "Aru", "Mutsuki", "Kayoko")
    ...
    assert state_7.cards == ("Aru", "Haruka", "Hoshino", "Mutsuki", "Kayoko", "Hanako")
...
def test_hanako_proxy_behavior():
    ...
    # ゲージを最初から「3」持っている状態からスタート
    state_0 = compiler.build_initial_state(initial_cards).update(hanako_gauge=3)

    print("\n--- Testing Proxy Behavior ---")
    assert state_0.env["hanako_gauge"] == 3

    # --- Step 1: プロキシカードがハナコを代理実行 ---
    # 期待値: ゲージ3以上だが、代理実行なのでゲージは消費されず、プロキシカードは通常通りサイクルする
    state_1 = transition(state_0, 1)  # Use MockProxy from slot 1

    assert state_1.cards == ("Hanako", "Mutsuki", "Aru", "Kayoko", "Haruka", "MockProxy")
    assert state_1.env["hanako_gauge"] == 3  # ゲージが消費されていないこと！
    print("[OK] プロキシによる代理実行時、ハナコのゲージは消費されず通常サイクルしました。")

    # --- Step 2: 別の通常キャラがスキルを使用 ---
    # 期待値: ハナコ自身のパッシブが働き、ゲージが増加する（3 -> 5）
    state_2 = transition(state_1, 2)  # Use Aru from slot 2

    assert state_2.env["hanako_gauge"] == 5
