...
def test_hanako_full_behavior():
    ...
    initial_cards = ("Hanako_Swimsuit", "Aru", "Mutsuki", "Kayoko", "Haruka", "Hoshino")

    sim = Simulator(specs)
    state = sim.initialize_state(initial_cards)

    # --- Initial State ---
    assert state.cards == ("Hanako_Swimsuit", "Aru", "Mutsuki", "Kayoko", "Haruka", "Hoshino")
    assert state.env.get("hanako_gauge", 0) == 0
    print("\n[OK] Initial state is correct.")

    # --- Step 1: Use Hanako with insufficient gauge (0) ---
    state_1 = sim.play_by_name("Hanako_Swimsuit")
    assert state_1.cards == ("Kayoko", "Aru", "Mutsuki", "Haruka", "Hoshino", "Hanako_Swimsuit")
    ...
    assert state_2.cards == ("Kayoko", "Haruka", "Mutsuki", "Hoshino", "Hanako_Swimsuit", "Aru")
    ...
    assert state_3.cards == ("Kayoko", "Haruka", "Hoshino", "Hanako_Swimsuit", "Aru", "Mutsuki")
    ...
    assert state_4.cards == ("Hanako_Swimsuit", "Haruka", "Hoshino", "Aru", "Mutsuki", "Kayoko")
    ...
    assert state_5.cards == ("Hanako_Swimsuit", "Aru", "Hoshino", "Mutsuki", "Kayoko", "Haruka")
    ...
    assert state_6.cards == ("Hanako_Swimsuit", "Aru", "Mutsuki", "Kayoko", "Haruka", "Hoshino")
    ...
    state_7 = sim.play_by_name("Hanako_Swimsuit")
    assert state_7.cards == ("Hanako_Swimsuit", "Aru", "Mutsuki", "Kayoko", "Haruka", "Hoshino")
    ...
    state_8 = sim.play_by_name("Hanako_Swimsuit")
    assert state_8.cards == ("Hanako_Swimsuit", "Aru", "Mutsuki", "Kayoko", "Haruka", "Hoshino")
    ...
    state_9 = sim.play_by_name("Hanako_Swimsuit")
    assert state_9.cards == ("Kayoko", "Aru", "Mutsuki", "Haruka", "Hoshino", "Hanako_Swimsuit")
...
class MockProxySpec(CharacterSpec):
    ...
    def should_cycle(self, state: State, k: int, target: str = "") -> bool:
        ...
        """
        if self.target_name == "Hanako_Swimsuit":
...
def test_hanako_proxy_behavior():
    ...
    specs = [
        ...
        GenericSpec("Kayoko"),
        GenericSpec("Haruka"),
        MockProxySpec(target_name="Hanako_Swimsuit"),  # ハナコを代理実行するカード
    ]
    # ハナコ本人と、プロキシカードが両方手札にある状態を作る
    initial_cards = ("Hanako_Swimsuit", "MockProxy", "Aru", "Mutsuki", "Kayoko", "Haruka")
    ...
    assert state_1.cards == ("Hanako_Swimsuit", "MockProxy", "Aru", "Mutsuki", "Kayoko", "Haruka")
    ...
    state_2 = sim.play_by_name("Aru")
    assert state_2.cards == ("Hanako_Swimsuit", "MockProxy", "Mutsuki", "Kayoko", "Haruka", "Aru")
    assert state_2.env["hanako_gauge"] == 200
    print("[OK] 通常キャラのスキル使用時、ハナコのゲージが正しくキャップされました。")

    # --- Step 3: ハナコ本人がスキルを使用 ---
    # 期待値: ゲージを100消費して手札に留まる
    state_3 = sim.play_by_name("Hanako_Swimsuit")
    assert state_3.cards == ("Hanako_Swimsuit", "MockProxy", "Mutsuki", "Kayoko", "Haruka", "Aru")
    ...
    assert state_4.cards == ("Hanako_Swimsuit", "MockProxy", "Mutsuki", "Kayoko", "Haruka", "Aru")
