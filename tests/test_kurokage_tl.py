...
def test_phase1() -> None:
    ...
    assert state.cards == ("Aru", "Kisaki", "Kikyou", "Sena", "Rio", "Hanako")
    assert state.get_env("hanako_gauge", 0) == 0
...
def test_phase2() -> None:
    ...
    開始時: cards=("Aru", "Kisaki", "Kikyou", "Sena", "Rio", "Hanako"), hanako_gauge=0
    終了時: hanako_gauge=160, Hanakoは手札に在中
    """
    print("\n=== フェーズ2 ===")
    sim = build_simulator()
    sim.state = sim.initialize_state(
        ("Aru", "Kisaki", "Kikyou", "Sena", "Rio", "Hanako"),
        env={"hanako_gauge": 0},
    )

    state = sim.play(0)  # Aru
    assert state.cards == ("Sena", "Kisaki", "Kikyou", "Rio", "Hanako", "Aru")
    assert state.get_env("hanako_gauge", 0) == 40
    print("[OK] Step 11: Aru, ゲージ40")

    state = sim.play(0)  # Sena
    assert state.cards == ("Rio", "Kisaki", "Kikyou", "Hanako", "Aru", "Sena")
    assert state.get_env("hanako_gauge", 0) == 80
    print("[OK] Step 12: Sena, ゲージ80")

    state = sim.play(0, target="Hanako")  # Rio → Rio_Copy
    assert state.cards == ("Rio_Copy", "Kisaki", "Kikyou", "Hanako", "Aru", "Sena")
    assert state.get_env("hanako_gauge", 0) == 120
    print("[OK] Step 13: Rio → Rio_Copy, ゲージ120")

    state = sim.play(1)  # Kisaki
    assert state.cards == ("Rio_Copy", "Hanako", "Kikyou", "Aru", "Sena", "Kisaki")
    assert state.get_env("hanako_gauge", 0) == 160
    print("[OK] Step 14: Kisaki, ゲージ160")

    state = sim.play(0)  # Rio_Copy（ハナコ手札在中のため即ドロー不発）
    assert state.cards == ("Aru", "Hanako", "Kikyou", "Sena", "Kisaki", "Rio")
    assert state.get_env("hanako_gauge", 0) == 160
...
def test_phase3() -> None:
    ...
    開始時: cards=("Aru", "Hanako", "Kikyou", "Sena", "Kisaki", "Rio"), hanako_gauge=160
    終了時: hanako_gauge=80, Hanakoは山札内
    """
    print("\n=== フェーズ3 ===")
    sim = build_simulator()
    sim.state = sim.initialize_state(
        ("Aru", "Hanako", "Kikyou", "Sena", "Kisaki", "Rio"),
        env={"hanako_gauge": 160},
    )

    state = sim.play(2)  # Kikyou
    assert state.cards == ("Aru", "Hanako", "Sena", "Kisaki", "Rio", "Kikyou")
    assert state.get_env("hanako_gauge", 0) == 200
    ...
    assert state.cards == ("Kisaki", "Rio", "Sena", "Kikyou", "Aru", "Hanako")
    assert state.get_env("hanako_gauge", 0) == 40
    print("[OK] Step 20: Hanako(3連目), ゲージ不足で通常サイクル")

    state = sim.play(1, target="Hanako")  # Rio → Rio_Copy
    assert state.cards == ("Kisaki", "Rio_Copy", "Sena", "Kikyou", "Aru", "Hanako")
    assert state.get_env("hanako_gauge", 0) == 80
    print("[OK] Step 21: Rio → Rio_Copy, ゲージ80")

    state = sim.play(1)  # Rio_Copy（ハナコ手札在中のため即ドロー不発）
    assert state.cards == ("Kisaki", "Kikyou", "Sena", "Aru", "Hanako", "Rio")
    assert state.get_env("hanako_gauge", 0) == 80
...
def test_phase4() -> None:
    ...
    開始時: cards=("Kisaki", "Kikyou", "Sena", "Aru", "Hanako", "Rio"), hanako_gauge=80
    終了時: hanako_gauge=0, Hanakoが即ドローでスロット0に在中
    """
    print("\n=== フェーズ4 ===")
    sim = build_simulator()
    sim.state = sim.initialize_state(
        ("Kisaki", "Kikyou", "Sena", "Aru", "Hanako", "Rio"),
        env={"hanako_gauge": 80},
    )
    ...
    assert state.cards == ("Hanako", "Kikyou", "Aru", "Rio", "Sena", "Kisaki")
    assert state.get_env("hanako_gauge", 0) == 160
    ...
    assert state.cards == ("Rio", "Kikyou", "Aru", "Sena", "Kisaki", "Hanako")
    assert state.get_env("hanako_gauge", 0) == 60
    print("[OK] Step 26: Hanako(2連目), ゲージ不足で通常サイクル")

    state = sim.play(0, target="Hanako")  # Rio → Rio_Copy
    assert state.cards == ("Rio_Copy", "Kikyou", "Aru", "Sena", "Kisaki", "Hanako")
    assert state.get_env("hanako_gauge", 0) == 100
    print("[OK] Step 27: Rio → Rio_Copy, ゲージ100")

    state = sim.play(0)  # Rio_Copy（ハナコ手札外かつゲージ≥100のため即ドロー発動）
    assert state.cards == ("Hanako", "Kikyou", "Aru", "Sena", "Kisaki", "Rio")
    assert state.get_env("hanako_gauge", 0) == 0
...
def test_phase5() -> None:
    ...
    開始時: cards=("Hanako", "Kikyou", "Aru", "Sena", "Kisaki", "Rio"), hanako_gauge=0
    終了時: hanako_gauge=80, Hanakoは手札に在中
    """
    print("\n=== フェーズ5 ===")
    sim = build_simulator()
    sim.state = sim.initialize_state(
        ("Hanako", "Kikyou", "Aru", "Sena", "Kisaki", "Rio"),
        env={"hanako_gauge": 0},
    )
    ...
    state = sim.play(0)  # Kisaki
    assert state.cards == ("Rio", "Kikyou", "Aru", "Hanako", "Sena", "Kisaki")
    assert state.get_env("hanako_gauge", 0) == 80
    print("[OK] Step 31: Kisaki, ゲージ80")

    state = sim.play(0, target="Hanako")  # Rio → Rio_Copy
    assert state.cards == ("Rio_Copy", "Kikyou", "Aru", "Hanako", "Sena", "Kisaki")
    assert state.get_env("hanako_gauge", 0) == 120
    ...
    assert state.cards == ("Rio_Copy", "Kikyou", "Hanako", "Sena", "Kisaki", "Aru")
    assert state.get_env("hanako_gauge", 0) == 60
    ...
    state = sim.play(1)  # Kisaki
    assert state.cards == ("Rio_Copy", "Aru", "Hanako", "Kikyou", "Sena", "Kisaki")
    ...
    assert state.cards == ("Kikyou", "Aru", "Sena", "Kisaki", "Rio", "Hanako")
    assert state.get_env("hanako_gauge", 0) == 80
...
def test_phase6() -> None:
    ...
    開始時: cards=("Kikyou", "Aru", "Sena", "Kisaki", "Rio", "Hanako"), hanako_gauge=80
    終了時: hanako_gauge=0, Hanakoは山札末尾
    """
    print("\n=== フェーズ6 ===")
    sim = build_simulator()
    sim.state = sim.initialize_state(
        ("Kikyou", "Aru", "Sena", "Kisaki", "Rio", "Hanako"),
        env={"hanako_gauge": 80},
    )
    ...
    state = sim.play(1, target="Hanako")  # Rio → Rio_Copy
    assert state.cards == ("Kisaki", "Rio_Copy", "Sena", "Hanako", "Kikyou", "Aru")
    assert state.get_env("hanako_gauge", 0) == 200
    print("[OK] Step 43: Rio → Rio_Copy, ゲージ200(MAX)")

    state = sim.play(0)  # Kisaki
    assert state.cards == ("Hanako", "Rio_Copy", "Sena", "Kikyou", "Aru", "Kisaki")
    assert state.get_env("hanako_gauge", 0) == 200
    print("[OK] Step 44: Kisaki, ゲージ200維持(MAX)")

    state = sim.play(1)  # Rio_Copy（ハナコ手札在中のため即ドロー不発）
    assert state.cards == ("Hanako", "Kikyou", "Sena", "Aru", "Kisaki", "Rio")
    assert state.get_env("hanako_gauge", 0) == 200
    ...
    assert state.cards == ("Aru", "Kikyou", "Sena", "Kisaki", "Rio", "Hanako")
    assert state.get_env("hanako_gauge", 0) == 0
